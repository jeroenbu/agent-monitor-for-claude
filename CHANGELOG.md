...
- (Personal build only) A session's row menu gains "Peek at the last turns": a dialog with the session's last conversation turns - your prompts, the assistant's replies, and a one-line marker per tool call - refreshed live while it stays open. This deliberately goes beyond the upstream project's content-free promise; see the personal-build note at the top of PRIVACY.md.
